...
class DiskStore {
    ...
    internal func createPathFrom<T>(_ type: T.Type, parentId: Int? = nil) -> String where T : WObject {
    ...
    func persist<T>(_ wobject: T) where T : WObject {
    ...
    func persist<T>(_ wobject: Set<T>) where T : WObject {
        let path = createPathFrom(T.self)
    ...
    func persist<T>(_ wobject: Set<T>?, parentId: Int) where T : WObject {
    ...
    func load<T>(_ type: [T].Type) -> Set<T>? where T : WObject {
        let path = createPathFrom(T.self)
    ...
    func load<T>(_ type: [T].Type, parentId: Int) -> Set<T>? where T : WObject {
        let path = createPathFrom(T.self, parentId: parentId)
        guard let wobject = try? Disk.retrieve(path, from: directory, as: [T].self) else {
            return nil
        }
